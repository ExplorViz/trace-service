...
public class TraceAggregator {
  ...
  private Trace initTrace(Trace freshTrace, SpanDynamic firstSpan) {
   ...
   */
  public Trace aggregate(Trace aggregate, SpanDynamic newSpan) {

    if (aggregate.getSpanList() == null || aggregate.getSpanList().isEmpty()) {
      return initTrace(aggregate, newSpan);
    }
