// Package spanproc is concerned with processing incoming spans.
package spanproc
...
import (
	...
	"github.com/ExplorViz/trace-service/internal/encoding"
...
// A spanCache stores previously encountered span IDs to avoid duplicate work.
// Should not be initalized directly; use [newSpanCache] instead.
...
func (s *spanCache) add(landscapeTokenID string, spanID string) {
	...
	if !ok {
		s.m[landscapeTokenID] = make(map[string]struct{})
...
func Run(ctx context.Context, cl *kgo.Client, tv token.TokenValidator, logInterval time.Duration) {
	...
	slog.Info("listening for incoming traces")
...
func consumerWorker(ctx context.Context, spans <-chan *attrib.SpanReader, results chan<- *spanpb.ParsedSpan, sc *spanCache, tv token.TokenValidator) {
	for {
		...
		case sr := <-spans:
			...
			p, err := parsing.ParseSpan(sr)
			if err != nil {
				slog.Error("failed to parse span", "error", err)
				continue
			}
			sc.add(tokenID, sr.SpanID())
			if pb, err := encoding.ToProtobuf(p); err != nil {
				slog.Error("failed to convert to protobuf", "error", err)
			} else {
				results <- pb
			}
		}
...
func validate(sr *attrib.SpanReader, tv token.TokenValidator) error {
	...
	if t.ID == "" {
		return fmt.Errorf("missing landscape token ID attribute (%s)", attrib.ExplorVizAttributes.LandscapeTokenID.Key)
	}
